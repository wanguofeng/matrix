...
static void wait_rsp_timeout(struct l_timeout *timeout, void *user_data)
{
	...
	if (IS_GROUP(req->addr) || IS_ALL_NODES(req->addr) || IS_VIRTUAL(req->addr)) {
		bt_shell_printf("\nSubscription address %x doesn't need to wait response\n", req->addr);
	} else {
...
void print_byte_array(const char *prefix, const void *ptr, int len)
{
	...
	if (prefix) {
		line = malloc(strlen(prefix) + (16 * 3) + 2);
...
static bool generic_onoff_send(uint8_t *buf, uint16_t len, uint32_t opcode)
{
	...
	uint16_t app_idx = mesh_db_node_model_bind_app_idx(0x0001, 0x0001, false, GENERIC_ONOFF_CLIENT_MODEL_ID);

	bt_shell_printf("Model ID\t%4.4x \"%s\"\n", GENERIC_ONOFF_CLIENT_MODEL_ID,
					sig_model_string(GENERIC_ONOFF_CLIENT_MODEL_ID));
...
static const struct bt_shell_menu generic_onoff_client_menu = {
	...
	.entries = {
	{"target", "<address>", cmd_dst_set,
				"Set target node to control onoff"},
